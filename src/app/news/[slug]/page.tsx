...
import { news, getNewsArticle, getAllNewsSlugs } from "@/data/news";
...
export default async function NewsDetailPage({ params }: Props) {
  ...
  // 前後の記事を取得
  const currentIndex = news.findIndex((a) => a.slug === slug);
  const prevArticle = currentIndex < news.length - 1 ? news[currentIndex + 1] : null;
  const nextArticle = currentIndex > 0 ? news[currentIndex - 1] : null;
